...
export function TopBar({ title, subtitle, actions }: TopBarProps) {
  return (
    <header className="h-14 border-b border-border bg-white/80 backdrop-blur-sm sticky top-0 z-20 px-6 flex items-center gap-4">
      {/* Title */}
      ...
      {/* Search */}
      <div className="hidden md:flex items-center gap-2 bg-canvas-100 border border-border rounded-lg px-3 py-1.5 w-56 group focus-within:ring-2 focus-within:ring-violet-500 focus-within:bg-white transition-all">
        ...
        />
        <kbd className="text-2xs text-ink-tertiary bg-white border border-border rounded px-1 py-0.5 font-mono hidden group-focus-within:hidden">
          ⌘K
        </kbd>
      </div>
      ...
      <button className="relative p-2 rounded-lg hover:bg-canvas-100 transition-colors">
        ...
        {pendingApprovals > 0 && (
          <span className="absolute top-1 right-1 w-4 h-4 bg-violet-600 text-white text-2xs font-bold rounded-full flex items-center justify-center">
            {pendingApprovals}
          </span>
        )}
      </button>
