...
const STEPS = [
  ...
  { id: "role",      title: "Role & responsibilities",     desc: "Understand your responsibilities, deliverables, and expectations.",                          icon: UserCheck,     done: false },
      ...
      <div className="p-6 max-w-3xl space-y-8 animate-in">
        <div className="card p-6 bg-gradient-to-br from-gold-50 to-canvas-100 border-gold-200">
          <div className="flex items-start justify-between gap-6">
            <div>
              ...
              <h2 className="text-heading">{ROLE_LABELS["CREATIVE_ASSISTANT"]}</h2>
            ...
            <div className="text-right flex-shrink-0">
              ...
              <p className="text-xs text-ink-tertiary mt-0.5">{completed} / {STEPS.length} complete</p>
            </div>
          </div>
          <div className="mt-4">
        ...
        <div className="space-y-3">
          {STEPS.map((step, i) => {
            ...
            const isNext = !step.done && STEPS[i - 1]?.done !== false;
            return (
              <div
                key={step.id}
                className={cn(
              ...
              >
                <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0", step.done ? "bg-emerald-50" : isNext ? "bg-gold-100" : "bg-canvas-100")}>
                  {step.done
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {isNext && (
                      <span className="text-2xs font-semibold px-1.5 py-0.5 rounded-full bg-gold-100 text-gold">Up next</span>
                    )}
                  </div>
                  <p className="text-xs text-ink-secondary mt-0.5">{step.desc}</p>
                </div>
                {!step.done && (
                  <Button size="sm" variant={isNext ? "primary" : "ghost"} rightIcon={<ChevronRight className="w-3.5 h-3.5" />} disabled={!isNext}>
